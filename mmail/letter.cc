...
letter_header::letter_header(mmail *mmA, const char *subjectA,
    ...
    net_address &netAddrA, bool charsetA, const char *newsgrpsA,
...
void letter_list::relist()
{
    ...
    while (noOfLetters && !noActive) {
        ...
        for (currentLetter = 0; currentLetter < noOfLetters; currentLetter++)
            if (!filter || filterCheck(filter)) {
                stat = getStatus();

                if ((llmode == 0) || ((llmode == 1) && !(stat & MS_READ)) ||
...
void letter_list::resort()
{
    if (lsorttype == LS_TO)
        ...
        if ((lsorttype == LS_TO) && ((type & INTERNET) && !(type & NETMAIL)))
            lsorttype = LS_SUBJ;
    }
    sort();
