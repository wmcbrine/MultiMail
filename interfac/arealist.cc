...
void AreaListWindow::MakeActive()
{
    ...
    list = new InfoWin(list_max_y + 5 + hasSys + hasProg, list_max_x + 2,
                       2, borderCol, tmp, C_ALBTEXT, 5 + hasSys + hasProg);
...
void AreaListWindow::Prev()
{
    do {
        Move(KEY_UP);
        Select();
    } while (!mm.areaList->getNoOfLetters() && (mm.areaList->getActive() > 0));
}
...
void AreaListWindow::Next()
{
    do {
        ...
        Select();
             ...
             (mm.areaList->getActive() < mm.areaList->noOfActive() - 1));
}
    ...
    case MM_MOUSE:
        {
            ...
            if ((mm_mouse_event.y != begy) || ((mm_mouse_event.x < (begx + 13))
                || (mm_mouse_event.x > (begx + 40))))
                break;
        }
#endif
    case 'L':
